...
class _SyncDetailsScreenState extends State<SyncDetailsScreen> {
  ...
  Future<void> _handleImport() async {
        ...
        return AlertDialog(
          ...
          title: Row(
            children: [
              ...
              const SizedBox(width: 12),
              const Expanded(
                child: Text('استيراد من ملف JSON',
                    style: TextStyle(
          ...
          content: const Text(
            'سيتم مسح جميع البيانات المحلية واستبدالها بالبيانات الموجودة في الملف.\n\n'
            'تأكد أن الملف صادر من لوحة تحكم Elegant Store.',
    ...
    setState(() {
      _isImporting = true;
      _importProgress = 0.0;
      _importStatus = 'جاري اختيار الملف…';
    });
    try {
      final syncService = context.read<SyncService>();
      await syncService.importFromFile(
        onProgress: (progress, status) {
          if (mounted) {
            setState(() {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context)
          ...
          ..showSnackBar(
            SnackBar(
              content: Text(_importStatus.isNotEmpty
                  ? _importStatus
                  : 'تم الاستيراد بنجاح ✓'),
              backgroundColor: Colors.deepPurple,
              duration: const Duration(seconds: 6),
      ...
      builder: (context, syncService, _) {
        ...
        return Column(
          ...
          children: [
            ...
            // ── Export JSON button ───────────────────────────────────────
            ElevatedButton.icon(
              onPressed: (_isResetting || isRestoring || _isExporting || _isImporting) ? null : _handleExport,
              icon: _isExporting
                  ? const SizedBox(
                      width: 18,
                      height: 18,
            ...
            // ── Import progress bar (visible only during import) ────────────
            if (_isImporting) ...[  
              Container(
                padding: const EdgeInsets.all(16),
                decoration: BoxDecoration(
            ...
            ElevatedButton.icon(
              onPressed: (_isResetting || isRestoring || _isExporting || _isImporting)
                  ? null
                  : _handleImport,
              icon: _isImporting
                  ? const SizedBox(
              ...
              label: Text(
                _isImporting ? 'جاري الاستيراد...' : 'استيراد من ملف JSON',
                style: const TextStyle(
                    fontWeight: FontWeight.bold,
                    color: Colors.white,
                    fontSize: 15),

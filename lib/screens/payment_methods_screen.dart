...
class _PaymentMethodsScreenState extends State<PaymentMethodsScreen> {
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              '$count ${count == 1 ? 'طريقة دفع غير فعّالة' : 'طرق دفع غير فعّالة'} — لن تظهر في شاشة البيع',
  ...
  Widget _buildMethodsGrid(bool isDark, bool isMobile) {
    return GridView.builder(
      ...
      gridDelegate: SliverGridDelegateWithMaxCrossAxisExtent(
        ...
        mainAxisExtent: isMobile ? 170 : 180,
        crossAxisSpacing: 12,
        mainAxisSpacing: 12,
      ),
  ...
  Widget _buildMethodCard(PaymentMethod method, bool isDark) {
    final (icon, color) = _typeIconColor(method.type);
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                Text(
                  isInactive ? 'غير فعّال' : 'فعّال في البيع',
                  style: TextStyle(
                      fontSize: 11,
                      color: isInactive
                          ? const Color(0xFF1E3A5F)
  ...
  String _typeLabel(String type) {
    return _types
            .firstWhere((t) => t['value'] == type,
                orElse: () => {'label': type})['label'] ??
        type;
  }

  (IconData, Color) _typeIconColor(String type) {
    switch (type) {
      case 'cash':
        return (Icons.money_rounded, Colors.green);
      case 'app':
        return (Icons.smartphone_rounded, const Color(0xFF0B74FF));
      case 'deferred':
        return (Icons.timer_outlined, const Color(0xFF1E3A5F));
      case 'credit_balance':
        return (Icons.account_balance_wallet_rounded, Colors.purple);
      case 'unpaid':
        return (Icons.warning_amber_rounded, Colors.redAccent);
      default:
        return (Icons.payment_rounded, Colors.grey);
    }
  }

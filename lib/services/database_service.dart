...
class DatabaseService {
  ...
  Future<Map<String, double>> getMonthlySales(int y, int m) async {
    ...
    final r = await db.rawQuery('SELECT SUBSTR(created_at, 1, 10) as day, SUM(amount) as total FROM invoices WHERE created_at LIKE ? AND deleted_at IS NULL GROUP BY day', ['$monthStr%']);
  ...
  Future<Map<String, dynamic>> getSalesStatsToday() async {
    final today = DateFormat('yyyy-MM-dd').format(DateTime.now());
    final db = await database;
    final s = await db.rawQuery('SELECT SUM(amount) as t FROM invoices WHERE created_at LIKE ? AND deleted_at IS NULL', ['$today%']);

...
class DatabaseService {
  ...
  Future<Database> initDatabase() async {
    ...
    final db = await openDatabase(
      ...
      onUpgrade: (db, oldVersion, newVersion) async {
        ...
        if (oldVersion < 6) {
          // 1. حذف كافة الفواتير والعمليات وتصفير الأرصدة (طلب المستخدم)
          await resetAllTransactions(db);
          
          // 2. إصلاح مشكلة الـ UNIQUE في جدول طرق الدفع عبر إعادة إنشائه
  ...
  // --- دوال المستخدمين (Users/Customers) ---
  ...
  Future<void> updateUser(User newUser, User oldUser) async {
    ...
    await db.update('users', {
      'name': newUser.name, 'nickname': newUser.nickname, 'phone': newUser.phone,
      'is_permanent_customer': newUser.isPermanentCustomer, 'credit_limit': newUser.creditLimit,
      'balance': newUser.balance, 'notes': newUser.notes, 'transfer_names': newUser.transferNames,
  ...
  // --- دوال طرق الدفع (Payment Methods) ---
  ...
  Future<void> updateInvoice(Invoice i) async { 
    final db = await database; 
    await db.update('invoices', i.toMap(), where: 'id = ?', whereArgs: [i.id]);
  }

  Future<List<Invoice>> getInvoices({DateTime? start, DateTime? end, String? query}) async {
    final db = await database;
    String where = 'i.deleted_at IS NULL'; List<dynamic> args = [];
  ...
  Future<void> logEdit(int targetId, String type, String field, String oldVal, String newVal) async {
    ...
    await db.insert('edit_history', {
      'target_id': targetId, 'target_type': type, 'field_name': field,
      'old_value': oldVal, 'new_value': newVal, 'created_at': DateTime.now().toIso8601String()
    });
  }
